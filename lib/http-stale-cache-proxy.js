...
_.extend(Cacher.prototype, {
  ...
  handleCheckCache: function(cached) {
    if(cached) {
      this.respondFromCache(function(error) {
        if(error) {
          logger.info(this.requestLogId + ' - Error happend, proxy and cache');
          this.proxyAndCache(this.response);
        } else if (this.options.noRefresh) {
          logger.info(this.requestLogId + ' - Refresh prevented with option noRefresh:true');
        } else {
          logger.info(this.requestLogId + ' - No error, refresh cache');
          this.refreshCache();
        }
      }.bind(this));
    } else {
  ...
  proxyAndCache: function(response) {
    ...
    if (typeof target === 'function') {
      target = target(this.request);
    }
    (typeof this.options.requestAgent === 'function' ?
      this.options.requestAgent : this.proxy.proxyRequest
    )(this.request, response, _.extend({}, this.options, {
      buffer: this.buffer,
      target: target
    }));
  },
